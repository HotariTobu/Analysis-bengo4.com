...
# %%
qus['質問日時'] = [datetime.datetime.strptime(obj.replace(':', ''), '%Y-%m-%dT%H%M%S%z') for obj in qus['質問日時']]
...
ans['「ありがとう」までの時間'] = [dt1 - dt2 for (dt1, dt2) in zip(ans['「ありがとう」日時'], ans['回答日時'])]
ans['BAまでの時間'] = [dt1 - dt2 for (dt1, dt2) in zip(ans['BA日時'], ans['回答日時'])]
